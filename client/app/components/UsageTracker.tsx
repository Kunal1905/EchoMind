...
export function UsageTracker({ minutesRemaining, minutesTotal, isPremium, label }: UsageTrackerProps) {
  const used = minutesTotal - minutesRemaining;
  const percentage = minutesTotal > 0 ? (used / minutesTotal) * 100 : 0;
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      ...
      <div className={`px-3 py-1 rounded-full text-xs ${
        ...
        {isPremium ? 'Premium' : minutesRemaining <= 0 ? 'Limit Reached' : `${minutesRemaining} min left`}
      </div>
    </div>
  );
}
